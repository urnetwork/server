	...
	server.Db(ctx, func(conn server.PgConn) {
		result, err := conn.Query(ctx, `
            WITH epoch_sweeps AS (
                ...
                FROM transfer_escrow_sweep sweep
                LEFT JOIN transfer_contract contract ON
                    contract.contract_id = sweep.contract_id AND sweep.provider_payouts IS NULL
                    AND contract.stream_id IS NULL
                LEFT JOIN LATERAL (
                    SELECT
                ...
                WHERE $1 <= sweep.sweep_time AND sweep.sweep_time < $2
            ), provider_rows AS (
                SELECT sweep.contract_id, sweep.balance_id, sweep.network_id,
            ...
            SELECT COALESCE(client_id, '00000000-0000-0000-0000-000000000000'::uuid),
                ...
                BOOL_AND(COALESCE(client_id <> '00000000-0000-0000-0000-000000000000'::uuid
                    AND payout_byte_count >= 0 AND payout_nano_cents >= 0 AND client_count = 1
                    AND allocated_bytes = network_bytes AND allocated_revenue = network_revenue, false))
            FROM provider_rows
