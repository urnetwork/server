...
func hmacCutoverIncompatibleFinding(snapshot hmacCutoverSnapshot) finding {
	...
	return finding{
		...
		mechanism: "Under the reviewed Connect policy, API and resident signers emit standard HMAC over the stored-contract bytes at the configured cutoff. An actual legacy-only receiver rejects standard-signed contracts before payload forwarding. This aggregate matches that incompatibility pattern but does not attest a running receiver or signer. Passing claimed-compatible checks exclude only a universal shared-path failure; they do not rule out concurrent or cohort-specific prober, API, contract, route, or destination failures.",
		...
		context:   "The version is provider-claimed metadata, and this aggregate does not include failure classes or executable identity. A dark verdict can include synchronous tunnel construction, TLS authentication, or all_destinations_failed. The latter means no sampled response met its status/content contract, not necessarily zero bytes; successful tunnel construction does not establish later API, contract, or route usability. Preserve separate integrity failures and obtain bounded fixed-class plus exact receiver/signer evidence before per-provider attribution. Unversioned and ambiguous descriptions remain unknown, and self-reported description alone must never become a permanent exclusion. A failed blackhole verdict hard-excludes selection only while current; after its " + model.ProviderBlackholeCheckMaxAge.String() + " lifetime it deliberately expires and fails open, so it is not durable quarantine. This is a protocol compatibility and rollout decision, not a Proxy RAM or active-client hardware ceiling.",
		action:    "Obtain an explicit security/availability decision. The secure path upgrades legacy-only receivers or uses a separately approved quarantine while verifying remaining provider capacity; neither a self-reported description nor the expiring blackhole exclusion is durable authorization for permanent removal. Temporary compatibility must uniformly build every API and Connect signer from one reviewed shared Connect policy source, rebuild and safely promote the monitor from that same policy source, and explicitly accept the weak key-only MAC for a bounded period with a named sunset. That restores availability but is not final protocol closure. Durable per-client capability negotiation is an architecture change and requires separate approval; do not silently move the date, deploy only one signer surface, lengthen probe timeouts, or treat retries as recovery.",
		verify:    "For the secure path, no legacy-only receiver remains eligible and §2.8/§2.9 prove adequate healthy capacity. For an approved compatibility path, every API and Connect signer proves the same reviewed shared Connect source and the monitor is rebuilt and promoted from that exact policy source; the legacy cohort then produces successful current checks and settled destination bytes. In either case §2.19 completes a whole refresh inside the " + model.ProviderBlackholeCheckMaxAge.String() + " verdict lifetime and the compatible control stays healthy for two cadences. Temporary compatibility retains an open upgrade-and-sunset obligation until standard signing is safely restored; passing its availability gate is not final closure.",
...
func hmacCutoverReadinessFinding(snapshot hmacCutoverSnapshot) finding {
	...
	return finding{
		...
		mechanism: "A wall-clock signing change is safe only after every active receiver can verify the new shape. Claimed legacy metadata establishes a readiness risk. Missing current checks or insufficient passing compatible checks keep this sample below the incompatibility-pattern threshold; meeting it would still not establish exact executable ancestry or a per-provider failure cause.",
		...
		context:   "Unknown metadata is not treated as compatible. Conversely, a claimed old version is not by itself proof that the live process still runs old code; the dark/pass comparison is an aggregate pattern, not an HMAC-specific or runtime-attestation discriminator. A current compatible control below the behavioral threshold fails closed to readiness and cannot retroactively negate an earlier behaviorally confirmed incompatibility; a PAGE-to-WARN class reassignment is not recovery.",
