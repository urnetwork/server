...
- GOTCHA — one task family can still contain several causes. On 2026-08-30,
  ...
  (§5.7).
- GOTCHA — wallet insufficiency is an operational liquidity boundary, not a
  task-scheduler defect. Consecutive task errors already back off to a one-hour
  cap. On 2026-08-31, 824 parked `AdvancePayment` rows therefore predicted
  about 13.7 retries/minute, matching the observed 8–16/minute log range. The
  rate is retry amplification, not unique payouts. Finance/ops must fund the
  exact network/token wallet or pause payouts; a software deploy cannot create
  liquidity. Never accelerate the retries, delete pending rows, or rotate
  idempotency keys to silence this class.
...
| `asset amount owned by the wallet is insufficient` / `insufficient token balance ... in wallet` (taskworker, Circle payment path) | The payout wallet cannot cover pending payouts (USDC on Solana — mint EPjFWdd5...Dt1v in the protected source log). This is not an API, PostgreSQL, or task-scheduler defect: each affected `AdvancePayment` remains pending on consecutive-error backoff capped at one hour, so N parked rows produce roughly N retry lines/hour. Alert artifacts redact wallet/entity ids. | **Finance/ops action required:** fund the exact network/token wallet from protected logs or pause payouts with the supported operational control. Code cannot create liquidity. Allow one backoff cap plus ingestion delay for natural convergence; never delete/manual-replay task rows, rotate payment idempotency keys, or accelerate retries. |
