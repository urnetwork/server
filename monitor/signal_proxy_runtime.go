...
func (proxyRuntimeProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	if len(lifecycleUnverified) > 0 {
		findings = append(findings, finding{
			...
			action:   "Deploy Proxy from intentional local server and SDK checkouts containing commit 04c40524 (and a4a8b502's manager/NetworkSpace join) and SDK commit e05ec46. Use the ordinary host-serialized rollout; no xops playbook is needed. Retain each checkout base and participating diff because the server Go VCS stamp alone does not prove the SDK source. Do not restart the same artifact or raise rollout concurrency to hide the gap.",
			verify:   "For two consecutive scrapes, every newest Proxy identity reports lifecycle_join_enabled=1 with valid source/digest identity. During a controlled drain, the old manager/device/RPC graph joins before the stop timeout, with no late device publication or continuing API refresh; simultaneous WireGuard plus HTTP/SOCKS acceptance stays healthy, old/candidate RSS overlap remains bounded, and no OOM or adjacent UDP receive loss appears.",
			playbook: "SIGNALS.md §14.7b",
		})
	}
	if len(high) > 0 {
		findings = append(findings, finding{
			...
			action:   "Provenance-check the selected source revision and modified state. If its recorded checkout lacks current-main server commit 04c40524, deploy from an intentional local checkout containing a11ae7b1's no-target Proxy warmup, value-only WireGuard TUN factory, bounded caller-lock cache, a4a8b502's complete manager/NetworkSpace join, and 04c40524's lifecycle capability gauge; build it against an intentional local SDK checkout containing e05ec46 so DeviceLocal, provider, RPC, remote, generator, sampler, monitor, and owned-API workers also join. The server Go VCS stamp does not prove the SDK checkout, so retain that dependency base and diff in release provenance. If a capability-proven 04c40524 artifact still crosses the band, capture an aggregate heap allocation profile or add identity-free owner gauges on that exact generation and separate full-sync payloads, shared NetworkSpace, hosted DeviceLocal structure, and active HTTP/SOCKS flows. Preserve pool/device metrics as negative controls. Do not force a production GC, restart away the evidence, raise GOGC, lower a cgroup below the measured live set, or shrink WireGuard queues without packet-ordering and slow-peer isolation tests.",
			verify:   "Deploy the Proxy root-cause artifact through the host-serialized rollout. For 15 minutes across multiple GC cycles, every newest identity reports lifecycle_join_enabled=1 with valid source/digest identity attributable to checkouts containing server 04c40524 and SDK e05ec46; peer/device counts and simultaneous WireGuard plus HTTP/SOCKS acceptance remain healthy, the startup heap/object floor is materially lower with conservative residual below 2 GiB, and RSS/host reserve improve with no new OOM or adjacent UDP receive drops. On a controlled drain, the old manager/device/RPC graph joins before the stop timeout with no late device publication or continuing API refresh.",
			playbook: "SIGNALS.md §14.7b",
		})
	}
	if len(findings) == 0 {
