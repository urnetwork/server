...
func (proxyRuntimeProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	if len(lifecycleUnverified) > 0 {
		findings = append(findings, finding{
			...
			action:   "Deploy Proxy from a clean current-main server descendant of commit 04c40524 (containing a4a8b502's manager/NetworkSpace join and the capability gauge), built against a clean SDK descendant of e05ec46. Use the ordinary host-serialized rollout; no xops playbook is needed. Retain the dependency checkout in release provenance because the server Go VCS stamp alone does not prove the SDK revision. Do not restart the same artifact or raise rollout concurrency to hide the gap.",
