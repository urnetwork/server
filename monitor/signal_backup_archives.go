...
func evaluateBackupArchiveRun(observation backupArchiveWriterObservation) finding {
	...
	action := "Read the bounded unit journal, verify the mounted archive and both direct SSH endpoints, then correlate the failure timestamp with each source sshd journal before changing rsync. A long source session with no orderly close plus a sibling forward reset before authentication localizes the shared public-forward path; a source-side close or error localizes that host instead. Repair the first proven dependency. If systemd has no retry scheduled, obtain operator authorization before starting one catch-up run."
	...
	if observation.remoteUnitSubstate == "auto-restart" {
		...
		action = "Do not manually start or duplicate the unit while its bounded on-failure retry is scheduled. Preserve the rsync partial, read the bounded journal, and correlate the failure timestamp with both source sshd journals before changing rsync: a long source session with no orderly close plus a sibling forward reset before authentication localizes the shared public-forward path. Verify both direct SSH forwards and observe the next systemd-owned attempt. Escalate the shared route or the individually proven source failure if that attempt cannot sustain progress."
	}
	return finding{
