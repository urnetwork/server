...
func evaluateBackupArchiveRun(observation backupArchiveWriterObservation) finding {
	...
	action := "Read the bounded unit journal, verify the mounted archive and both direct SSH endpoints, then correlate the failure timestamp with Planetoid's NetworkManager connectivity state, an unrelated public control such as its VPN endpoint, and each source sshd journal before changing rsync. Concurrent no-route failures to the backup endpoint and the independent public control localize Planetoid's router or upstream Internet path. If independent Internet stayed healthy, a long source session with no orderly close plus a sibling forward reset before authentication instead localizes the shared public-forward path; a source-side close or error localizes that host. Repair the first proven dependency. If systemd has no retry scheduled, obtain operator authorization before starting one catch-up run."
