// Shared PostgreSQL collection and evaluation used by the pg-state and
// contract-rate signals.
package monitor
...
func (self *pgStateProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	if idleInTx > 100 || oldestIdleS > 30*60 {
		findings = append(findings, finding{
			...
			symptom:   fmt.Sprintf("pg (%s) idle-in-transaction = %d, oldest transaction age %ds (threshold > 100 / > 30m)", target, idleInTx, oldestIdleS),
	...
	// zombie-tx is the long-xact-age half of 1.3 split into its own stable
	// identity because its mechanism (vacuum xmin pin) differs from a count
	// storm caused by Redis latency.
	if oldestIdleS > 30*60 {
		findings = append(findings, finding{
			probeId: "pg/zombie-tx", tier: tierWarn,
			class: "zombie-tx", target: target, sustain: 1,
			symptom:   fmt.Sprintf("pg (%s) oldest idle transaction is %ds old", target, oldestIdleS),
			mechanism: "A leaked transaction pins the cluster-wide xmin horizon and can silently prevent autovacuum from reclaiming dead tuples.",
			baseline:  "Oldest idle-in-transaction age is below one minute; warn above 30 minutes.",
			observed:  fmt.Sprintf("oldest_idle_s=%d idle_in_tx=%d", oldestIdleS, idleInTx),
			evidence:  idleTxBattery(ctx, env),
			action:    "Identify the owning application/session and its last query before terminating only the proven leaked transaction.",
			verify:    "No idle transaction is older than 30 minutes and vacuum horizons advance.",
			playbook:  "SIGNALS.md §1.3 and §5.6",
		})
	} else {
		findings = append(findings, healthyFinding("pg/zombie-tx", tierWarn, "zombie-tx", target))
	}
	...
	rows, err := env.runner.pg(ctx, `
		WITH idle AS MATERIALIZED (
			SELECT pid, coalesce(application_name,'') AS application_name,
			       coalesce(round(extract(epoch FROM now()-state_change)),0) AS idle_s,
			       left(regexp_replace(coalesce(query,''),'\s+',' ','g'),160) AS last_query
			FROM pg_stat_activity
			WHERE backend_type='client backend' AND state='idle in transaction'
		), oldest AS (
			SELECT * FROM idle ORDER BY idle_s DESC, pid LIMIT 1
		), shapes AS (
			SELECT count(*) AS backends, max(idle_s) AS oldest_s,
			       left(last_query,90) AS last_query
			FROM idle
			GROUP BY left(last_query,90)
			ORDER BY backends DESC, oldest_s DESC, last_query LIMIT 6
		)
		SELECT 0 AS sort_key, 'oldest' AS kind, 1 AS backends, idle_s AS oldest_s,
		       pid, application_name, last_query
		FROM oldest
		UNION ALL
		SELECT 1, 'shape', backends, oldest_s, 0, '', last_query
		FROM shapes
