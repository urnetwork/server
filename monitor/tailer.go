...
var logClasses = []logClass{
	...
	{name: "loki-tail-backend-eof", re: regexp.MustCompile(`caller=tail\.go:[0-9]+\s+component=tail-querier\b.*\bmsg="Error receiving response from grpc tail client"(?:\s+addr=(?:"[^"]+"|[^[:space:]]+))?\s+err=EOF(?:\s|$)`),
		...
		action:    "Provenance-gate Warp commits 1e95aef and bca37cf and deploy them only to older Grafana blocks. For the already-current fleet, deploy one Grafana image containing Warp commit 5927527. It includes 13fcd05's bounded self-telemetry reduction and forwards the ingester's bounded drop descriptors through Loki's existing HTTP dropped_entries response, allowing the affected standing service tail to be named without exposing labels or timestamps. Do not claim producer reduction or attribution itself fixes loss. Correlate a residual service-attributed drop with backend EOFs and the same window's reset records, then inspect the named backend's tailer, process, ring, and network state. Do not raise Loki's fixed queues, tail-request limits, or ring session cap, and do not restart the same image.",
