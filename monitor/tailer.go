...
var logClasses = []logClass{
	...
	{name: "netescrow-negative", re: regexp.MustCompile(`\[netescrow\]negative counter after`),
		...
		action:    "Do not manually zero/delete individual mirrors or invoke reconciliation. Confirm the exact executor has the transfer_escrow(balance_id, contract_id) index, page-local additive reconciler, and atomic release Lua. Retain them where present and roll them out only where version or code evidence says they are absent. If every residual line says clamped_to=0 and the matching aggregate stays small, observe the contained commit/post ordering window through a full quiet interval; if the rate persists, trace that ordering rather than redeploying the already-present fixes.",
