...
var logClasses = []logClass{
	...
	{name: "payment-processor-rate-limit", re: regexp.MustCompile(`Bad status: 429 Too Many Requests.*API rate limit error`),
		...
		action:    "Do not manually retry, delete, or pull payment tasks forward. Deploy taskworker commit 70b0d269 or later so saturated retries disperse across 30–90 minutes. If 429s recur after complete convergence and one full 90-minute drain window, measure exact per-second Create/Get traffic across every taskworker and any other Circle client against the account's authoritative quota before adding or tuning a shared provider limiter.",
