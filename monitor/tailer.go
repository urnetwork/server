...
var logClasses = []logClass{
	...
	{name: "connect-tls-disabled", re: regexp.MustCompile(`\[c\]Could not initialize tls config\. Disabling transport\. = `),
		...
		mechanism: "Before server commit 64366fb5, Connect logged this error and continued with an empty TransportTls. Listener supervision could then bind the configured UDP sockets and report readiness even though GetConfigForClient could not select a certificate, so every H3/H3DNS handshake failed below authentication with TLS internal_error. The corrected checked constructor cancels startup and returns the identity error before any listener goroutine begins.",
		context:   "This is an affirmative silent-readiness defect in the emitting Connect-bearing process, not generic provider unresponsiveness and not a Proxy hardware-capacity ceiling. A bounded seven-day main search on 2026-09-02 found no occurrence, so the fix is required correctness hardening but this class does not explain that day's provider degradation without a matching line.",
		action:    "Inspect the emitting process's active TLS resource and certificate/key load error without copying key material into logs. Repair the configuration source, then deploy a server descendant of 64366fb5 so an invalid identity fails startup instead of advertising a dead UDP listener. Do not restart the same artifact repeatedly, weaken certificate loading, or treat a bound socket as recovery.",
		verify:    "Every Connect-bearing artifact contains server commit 64366fb5; a deterministic invalid-loader test starts no listener; each valid running allocation reports listener readiness and completes a real QUIC handshake on every enabled carrier; and connect-tls-disabled remains zero through two rollout cadences.",
		redactIDs: true},
	...
	{name: "loki-tail-backend-eof", re: regexp.MustCompile(`caller=tail\.go:[0-9]+\s+component=tail-querier\b.*\bmsg="Error receiving response from grpc tail client"(?:\s+addr=(?:"[^"]+"|[^[:space:]]+))?\s+err=EOF(?:\s|$)`),
		...
		mechanism: "The historical exact 59-61-second recurrence came from Warp's Grafana ring TCP proxy applying a 60-second application read deadline to long-lived HTTP/2 and gRPC connections; Warp commit 1e95aef removes it. The current off-grid EOF wave has a separate Loki 3.7.3 tail-handoff path. Each ingester tailer has a 100-stream processing queue and a five-stream send queue. Upstream receiveStreamsLoop discarded a stream immediately whenever the five-slot handoff was full, even while the 100-slot processing queue could absorb a short burst. Its first genuine overflow records blockedAt; continued traffic more than 15 seconds later closes that server-side tail. Ingester.Tail then returns nil, the querier's Recv observes EOF and drops the client, and its five-second connection ticker reconnects. A backend process exit or ring loss can still produce the same EOF text, so the backend address and paired reset evidence are required discriminators.",
		...
		action:    "Verify the running Grafana artifacts contain Warp commits 1e95aef and bca37cf, and deploy them only to older Grafana blocks. For the current short-burst loss, deploy one Grafana image containing Warp commit 35453fd. It contains 5927527's producer reduction and bounded descriptor forwarding, and changes only the five-slot ingester handoff to wait cancellation-aware for available capacity so the existing 100-slot processing queue can absorb a short burst. Correlate a residual service-attributed drop with backend EOFs and the same window's reset records, then inspect the named backend's tailer, process, ring, and network state. Do not raise Loki's fixed queues, tail-request limits, or ring session cap, and do not restart the same image.",
		verify:    "Every Grafana block contains Warp commit 35453fd; Mimir query-frontend/evaluator statistics and Loki table lookup info records are zero while alert rules, query metrics, warnings, and errors remain healthy; bucket-index version gaps converge below two minutes; every configured active ring member owns its LAN identity and heartbeats; and loki-tail-dropped-streams, service-attributed loki-tail-dropped-entries, plus loki-tail-backend-eof remain zero for 10 minutes with stable standing tails and complete bounded reconciliation. Genuine sustained blockage remains bounded and observable; any residual EOF must carry a backend frame and be reconciled against that exact node.",
	},
	...
	// forwards it. Warp 35453fd separately makes the five-slot handoff block
	// cancellation-aware so the existing 100-slot queue absorbs short bursts.
	// This raw log remains observation-only and must be paired with the
	// service-attributed Warpctl summary.
	{name: "loki-tail-dropped-streams", re: regexp.MustCompile(`caller=tailer\.go:[0-9]+\b.*\bmsg="tailer dropped streams is reset"(?:\s|$)`),
		...
		mechanism: "Loki 3.7.3 gives each ingester tailer a 100-stream processing queue and a five-stream send queue. Its upstream receiveStreamsLoop used a nonblocking send to the five-slot channel and dropped immediately when that channel was full, even while the 100-slot input queue had capacity. Warp 35453fd replaces only that handoff with a blocking cancellation-aware select, so a burst larger than five but smaller than the processing bound drains without loss. Sustained blockage beyond the existing 100-slot bound still drops streams, retains bounded metadata, and can close the tail after 15 seconds; the querier then logs a backend EOF and reconnects on its five-second ticker. Warp 5927527, contained by 35453fd, separately forwards those bounded descriptors through pushTailResponseFromIngester without enlarging either queue.",
		...
		action:    "Deploy Warp commit 1e95aef and server Proxy commit e055c98c only to blocks whose artifacts predate them. For the current short-burst loss, deploy a Grafana artifact containing Warp 35453fd. It contains 13fcd05's producer reductions, 5927527's descriptor forwarding, and the cancellation-aware five-slot handoff fix without changing queue sizes. Retain bounded reconciliation continuation and pair this observation-only reset with service-attributed loki-tail-dropped-entries after the new artifact is live. Do not raise Loki's fixed queues, suppress genuine sustained-backpressure evidence, or disable correctness reconciliation.",
		verify:    "Every Grafana and Proxy block contains the required fixes including Warp 35453fd, every Grafana ring node is healthy, Mimir query-frontend/evaluator statistics and Loki table lookup info records remain zero while alert rules, query metrics, warnings, and errors remain healthy, bucket-index version gaps converge below two minutes, default Proxy logs contain one aggregate summary per reconciling instance, every reconciliation partition drains through bounded continuation, and loki-tail-dropped-streams, service-attributed loki-tail-dropped-entries, plus loki-tail-backend-eof remain zero for 10 minutes with all external tails connected. A greater-than-100 sustained burst may still produce bounded descriptors; any residual EOF is framed by backend address.",
	},
	...
	{name: "conn-reset", re: regexp.MustCompile(`connection reset by peer|unexpected EOF`),
		rateThreshold: 50, tier: tierWarn, playbook: "SIGNALS.md §4 and §13",
		meaning:   "the receiving process observed an abrupt transport termination without an orderly close; neither the line nor its logger identifies which endpoint or network hop caused it",
		mechanism: "Connection reset by peer is peer-relative: a TCP RST reached the receiver. WebSocket close 1006 with unexpected EOF means the receiver reached the end of the byte stream without a WebSocket close frame. Depending on connection direction, the peer may be a device or another service, and a NAT, load balancer, firewall, or other intermediary can also discard state or inject a reset. These messages alone cannot distinguish peer lifecycle, path failure, remote process replacement, or resource pressure at either endpoint.",
		context:   "On 2026-09-03, Fireside Proxy emitted 87-158/min device_rpc_transport WebSocket close-1006 unexpected-EOF lines while the focused proxy-memory, proxy-pool, proxy-runtime, and proxy-cache controls were all healthy. A redacted ten-minute cohort contained 1,176 attach/EOF/detach triplets from one logical hosted device: 1,075 of 1,175 measured sessions lasted under one second, 100 lasted one to four seconds, and none reached five seconds. There were zero write, keepalive, receive-budget, receive-queue, version-rejection, instance-rejection, or SyncReverse markers. Proxy accepts this /device-rpc WebSocket and the remote DeviceRemote initiates it; the exact 500-millisecond pace matches failed-sync retry pacing and localizes this wave to one early remote-to-local sync lifecycle without yet identifying why sync failed. The contemporaneous controls reject Proxy resource pressure as the demonstrated cause; they do not make those controls universally irrelevant. This cohort predates the bounded endpoint markers now in source, so marker absence in that historical window is not evidence for a stage.",
		action:    "Identify whether the emitter accepted or initiated the connection, then inspect the exact peer and path for process-generation changes, orderly-close evidence, load-balancer/NAT/conntrack loss, carrier errors, OOM/cgroup pressure, socket pressure, and provider or device lifecycle. For the current Fireside cohort, preserve the redacted one-device cardinality and compare the exact remote and Proxy RPC wire generations. After both updated endpoints are deployed, pair the remote's one-per-attempt `[drpc-attempt] endpoint=remote stage=<dial|sync|sync-reverse|active> result=<bounded>` marker with Proxy's one-per-session `[drpc-session] endpoint=proxy stage=<transport|request|response> result=<bounded> ingress=<absent|present> egress=<absent|present>` marker. Proxy `stage=transport ingress=absent` proves its mux read no non-empty binary request from the socket; `stage=request egress=absent` proves request bytes were read but no non-empty response frame was successfully written; `stage=response` plus the remote stage distinguishes response receipt/decode from a later reverse-sync failure. The markers retain no ids, addresses, frame bytes, counts, or raw errors. Do not call the event a server close, restart a healthy process, increase buffers, or attribute it to memory without a matching direct signal.",
		verify:    "The conn-reset rate remains below 50/min for 10 minutes through equivalent traffic, the exact causal peer/path or resource signal remains healthy after its fix, and orderly connection replacement continues to work. A lower log rate without the causal control is not root-cause verification."},
	...
	{name: "grafana-plugin-unregistered", re: regexp.MustCompile(`plugin\.notRegistered|plugin not registered`),
		...
		context:   "A direct Mimir or Loki query, Grafana /api/health, or the datasource database row cannot exercise plugin loading. Correlate the bounded referer and request path with the exact Grafana generation, then run both authenticated /api/ds/query controls on that same generation. The dedicated grafana-datasources probe owns the native-plugin packaging diagnosis. Grafana's retained request log and plugin metrics do not expose the rejected datasource type, and centrally logging query bodies would collect more dashboard state than this diagnosis requires.",
		action:    "If vector(1) through warp-mimir or bounded count_over_time through warp-loki fails on the same generation, publish a corrected image with the pinned native plugin and catalog SHA-256 and run Warp's packaging and provisioning tests. If both controls succeed and both plugin processes remain present, discard or reload the stale originating browser/dashboard state. If the request persists, capture only queries[].datasource.type and queries[].datasource.uid locally in that browser's developer tools; do not add central request-body logging. Correct the unsupported caller. Do not recreate a healthy datasource, install plugins from the network at startup, silence the query error, or restart an unchanged image.",
		verify:    "Query vector(1) through warp-mimir and a bounded count_over_time query through warp-loki via Grafana /api/ds/query on every active exact-edge generation, confirm the request's dashboard or client now names a registered type and Logs Drilldown selects var-ds=warp-loki, observe a successful provisioned-rule evaluation, and require zero new grafana-plugin-unregistered lines for 10 minutes after log-ingestion delay.",
	},
	...
	for _, address := range addresses {
		...
		if monitorEvidence.matches(event) {
			...
			mechanism = fmt.Sprintf(
				"The monitor host recorded that its IPv6 default-router lifetime on %s reached zero at %s inside the transport-failure window, followed by loss of that interface's IPv6 network state at %s. The affected warpctl tail transport(s) reported route loss while their child processes remained alive. This proves local default-router expiration and supersedes edge attribution, but the configd record alone does not distinguish an explicit zero-lifetime Router Advertisement from missed or late refresh advertisements or local RA-state invalidation.",
			...
			contextText = "This event is a monitor-side first-hop IPv6 default-router state loss, not evidence that the named production edge, its interface, LB, or LAN neighbor failed. A later edge HTTP 200 is expected after the monitor's IPv6 route returns. The first-hop router, its RA/failover owner, the local advertisement delivery path, and the monitor's local RA-state owner remain distinct candidates; deploying an edge service cannot repair this local observation boundary."
			action = "Inspect the monitor's local first-hop IPv6 Router Advertisement path: correlate router and RA-daemon uptime, WAN/failover state, local-link health, and the monitor's stored-router state at the recorded time. With explicit operator authorization, capture timestamped ICMPv6 type 134 traffic on the monitor interface during recurrence. Correlate complete packet coverage with local state to distinguish an explicit zero-lifetime withdrawal, missed or late refresh advertisements, and local RA-state invalidation. An absent packet in incomplete capture is not proof of missed delivery. Select a router, delivery-path, or local-client repair only after that discriminator, not from configd text alone. Do not change the named production edge, its Vault address, LB, firewall, or neighbor state for this locally proven event."
