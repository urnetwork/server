...
var logClasses = []logClass{
	...
	// Loki's reset log proves the server filled its dropped-stream metadata
	// list. A fixed Warpctl also reports every non-empty dropped_entries tail
	// response directly, catching smaller losses and attributing them to the
	// affected service without copying labels or timestamps.
	{name: "loki-tail-dropped-streams", re: regexp.MustCompile(`(?:caller=tailer\.go:[0-9]+\b.*\bmsg="tailer dropped streams is reset"(?:\s|$)|^\[warpctl\]\[loki-tail-dropped-entries\]\s+service=[A-Za-z0-9._-]+\s+count=[1-9][0-9]*(?:\s|$))`),
		rateThreshold: 1, tier: tierWarn, playbook: "SIGNALS.md §1.5 and §4",
		meaning:   "a Loki live-tail consumer stayed blocked while its bounded queue overflowed, so records were omitted from the WebSocket stream; either Loki reset a full dropped-stream metadata list or Warpctl received a non-empty dropped_entries response",
		mechanism: "Loki queues only a bounded number of streams per tailer. If the client cannot consume a burst, new streams are dropped and described in tail-response metadata; filling the server's bounded metadata list emits the reset log. Older Warpctl decoded dropped_entries but silently discarded it, hiding losses smaller than a reset and losing affected-service attribution. On 2026-08-31, one Proxy block emitted 19,995 per-peer WireGuard installation info lines in one minute, while Grafana emitted 18,165 reset lines in the same minute and the Proxy and Grafana reconciliation queries both reached 20,000 rows.",
		context:   "This is affirmative loss of live-tail contents even when the external WebSocket process remains connected. Fixed Warpctl emits one privacy-safe service/count summary and omits dropped labels and timestamps. The standing range reconciliation is the recovery path, but a capped query is not proof of complete recovery. The observed Proxy burst was default info-log amplification, not 19,995 distinct peer-installation failures; the terminal peer summary and gauge remained the fleet-level health evidence.",
		action:    "Run the monitor with Warpctl containing Warp commit 26089b2, which surfaces non-empty dropped_entries responses. Remove or verbosity-gate the high-cardinality producer while retaining its aggregate summary and metrics. For the observed Proxy shape, deploy the server change that moves per-client peer-installation details to V(1). Retain bounded reconciliation continuation for any partition that reaches the row cap. Do not raise Loki tail queues or suppress either loss signal.",
		verify:    "During the next full Proxy peer synchronization, default logs contain one aggregate sync summary rather than one line per installed peer, every reconciliation partition completes through its bounded continuation, no direct dropped_entries summary appears, and loki-tail-dropped-streams remains zero for 10 minutes with all external tails connected.",
	},
