...
var logClasses = []logClass{
	...
	{name: "loki-tail-backend-eof", re: regexp.MustCompile(`caller=tail\.go:[0-9]+\s+component=tail-querier\b.*\bmsg="Error receiving response from grpc tail client"\s+err=EOF(?:\s|$)`),
		...
		mechanism: "The pre-fix incident's exact 59-61-second recurrence came from Warp's Grafana ring TCP proxy applying a 60-second application read deadline to long-lived HTTP/2 and gRPC connections. Warp commit 1e95aef removes that read deadline and uses TCP keepalive. The same EOF text is not cause-specific, however: a backend process exit, ring-member loss, or another blocked stream can also end the internal gRPC tail. Ring connection counts in the deadline incident remained far below the independent 256-session ceiling.",
		context:   "A quoted rpc Canceled/context-canceled error during explicit watcher retirement is a separate client lifecycle and this class deliberately excludes it. Post-fix production then emitted non-grid EOFs from current 2026.8.31+1034210530 containers while Fireside was absent from the active Loki ring because it had lost its LAN identity. That disproves blind attribution to the old 60-second deadline without proving that every new EOF has the same node-loss cause. Preserve the emitting artifact, cadence, ring membership, and grafana-node findings. Bounded log reconciliation remains required for late source timestamps and the Search-to-LiveTail handoff.",
		action:    "Inspect the emitting Grafana container's artifact and the host-local grafana-node/ring findings first. Deploy Warp commit 1e95aef only to a Grafana block whose image predates it. If the image already contains the fix, do not rebuild or restart it from this alert: restore any missing ring member or LAN identity, then capture the exact backend route if EOF persists with every node healthy. Do not raise Loki tail-request limits or the ring session cap.",
		verify:    "Every Grafana block contains Warp commit 1e95aef, every configured active ring member owns its LAN identity and heartbeats, no loki-tail-backend-eof line recurs for 10 minutes with stable standing tails, and each bounded two-minute reconciliation completes below its result cap.",
	},
	...
	{name: "loki-tail-dropped-streams", re: regexp.MustCompile(`caller=tailer\.go:[0-9]+\b.*\bmsg="tailer dropped streams is reset"(?:\s|$)`),
		...
		mechanism: "Loki 3.7.3 gives each ingester tailer a 100-stream processing queue and a five-stream send queue. A blocked internal gRPC send or a producer burst drops streams. After ten retained drop descriptors the ingester emits this reset. Its querier pushTailResponseFromIngester path forwards resp.Stream but discards resp.DroppedStreams, so HTTP dropped_entries and Warpctl cannot expose or attribute this earlier loss. On 2026-08-31, one Proxy block emitted 19,995 per-peer installation lines in one minute while Grafana emitted 18,165 resets. Later complete controls found 39,378, 38,513, and 39,583 periodic Proxy detail lines with matching reset waves; the final wave came from immutable pre-fix callsite server.go:764 before watcher promotion. After 2026.8.31+1034210530 deployed both the ring-deadline and Proxy-log fixes, a new one-minute window still returned 1,762 Grafana lines: 344 Mimir query-frontend stats, 344 evaluator stats, 252 Loki table-manager records, 238 reset records, and 175 Grafana Prometheus-plugin completions. Roughly 170 short alert-rule queries per minute had made the observation plane its own default-info producer. This post-fix source is distinct from the earlier Proxy wave.",
		context:   "This is affirmative internal live-tail loss even when the external WebSocket process remains connected. Grafana is the observation service that emitted the reset, not the affected application-tail identity. The standing range reconciliation across every service is the recovery path, but a capped service-wide query is not a total: repeat the same absolute window for each configured block and require every partition to drain. First provenance-gate both historical fixes; do not redeploy already-current blocks from historical prose. Current post-fix measurement also overlapped a missing Fireside LAN identity, so node recovery remains a prerequisite before attributing all residual resets to self-telemetry alone.",
		action:    "Deploy Warp commit 1e95aef and server Proxy commit e055c98c only to blocks whose artifacts predate them, and restore any active grafana-node LAN/ring failure. For already-current blocks, deploy a Grafana image containing Warp commit 42168fe, which disables only Mimir's per-query statistics stream while retaining query execution, metrics, errors, and alert cadence. Retain bounded reconciliation continuation. Do not raise Loki's fixed queues, suppress the reset, disable correctness reconciliation, or claim service attribution that Loki 3.7.3 does not preserve.",
