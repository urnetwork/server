...
var logClasses = []logClass{
	...
	{name: "loki-tail-backend-eof", re: regexp.MustCompile(`caller=tail\.go:[0-9]+\s+component=tail-querier\b.*\bmsg="Error receiving response from grpc tail client"(?:\s+addr=(?:"[^"]+"|[^[:space:]]+))?\s+err=EOF(?:\s|$)`),
		...
		action:    "Provenance-gate Warp commits 1e95aef and bca37cf and deploy them only to older Grafana blocks. For the already-current fleet, deploy one Grafana image containing Warp commit 13fcd05. It stops Mimir 3.1.1 routine info at the source while retaining warnings, errors, query metrics, and alert cadence; refreshes the single-tenant store-gateway bucket index every minute; and moves Loki's per-query table lookup record to debug while retaining reset and EOF evidence. Do not claim producer reduction itself fixes loss. If resets or EOFs remain, inspect the named backend's tailer, process, ring, and network state in the same window. Do not raise Loki's fixed queues, tail-request limits, or ring session cap, and do not restart the same image.",
		verify:    "Every Grafana block contains Warp commits 1e95aef, bca37cf, and 13fcd05; Mimir query-frontend/evaluator statistics and Loki table lookup info records are zero while alert rules, query metrics, warnings, and errors remain healthy; bucket-index version gaps converge below two minutes; every configured active ring member owns its LAN identity and heartbeats; and loki-tail-dropped-streams plus loki-tail-backend-eof remain zero for 10 minutes with stable standing tails and complete bounded reconciliation. Any residual EOF must carry a backend frame and be reconciled against that exact node.",
	},
	...
	{name: "payout-wallet-insufficient", re: regexp.MustCompile(`asset amount owned by the wallet is insufficient|insufficient token balance .* in wallet`),
		...
		burst: &logBurst{
			...
			action:    "Use §8.12 to verify every taskworker's immutable source and deploy a clean artifact containing current-main server commit 66525afc only to blocks that lack it. Preserve normal backoff and payment idempotency keys; do not accelerate, manually replay, or delete tasks. If every block already contains the gate, use §2.14 admission errors/waits and all Circle request sources before changing its ceiling.",
	...
	{name: "payment-processor-rate-limit", re: regexp.MustCompile(`Bad status: 429 Too Many Requests.*API rate limit error`),
		...
		action:    "Do not manually retry, delete, or pull payment tasks forward. Use §8.12 to verify every taskworker's immutable source and deploy a clean artifact containing current-main server commit 66525afc only to blocks that lack its fleet-wide Redis-time transfer gate and fail-closed telemetry. If every block contains it, inspect §2.14 fail-closed errors and admission pressure plus all other Circle request sources before changing the conservative three-per-second ceiling.",
	...
	{name: "db-maintenance-legacy-reindex", re: dbMaintenanceLegacyReindexRe,
		...
		action:    "Inspect the active database operation before changing Taskworker. Do not let a rollout or manual cancellation implicitly interrupt a protected rebuild. Operations must explicitly choose whether a bounded active operation should finish or be canceled; cancellation is a database mutation and requires authorization. Once pg_stat_progress_create_index is empty, satisfy §8.13 and deploy Taskworker from a clean descendant containing current-main server commits 908a8b2c and d8392c83. Clean existing artifacts separately with the supported cleanup-only maintenance command and explicit database-maintenance authorization; never wildcard-drop _ccnew/_ccold indexes.",
	...
	{name: "framer-message-too-large", re: framerRejectRe,
		...
		action:    "After the §8.13 release-builder gate is healthy, deploy clean Connect and Proxy artifacts containing current-main server commit c1403f16 and Connect module commit 096414ac (or clean descendants) on both ends of the hosted H1 path. Keep ordinary data coalescing and per-device memory admission bounded; do not suppress the rejection or make the buffer unbounded.",
		verify:    "§8.12 proves every active Connect and Proxy block runs the clean compatible commit pair; no framer-message-too-large line recurs; provider windows stay at target without platform-unreachable; and three sustained HTTP/SOCKS/WireGuard overlap campaigns pass.",
	},
