...
func (releaseBuilderProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	if len(invalid) > 0 {
		findings = append(findings, finding{
			...
			action:    "Stop release builds through every listed executable. Install a clean Warpctl built from Warp commit 217392e or a clean descendant, then rebuild affected service images from clean source. Do not retag or reuse an unverifiable binary or infer source from WARP_VERSION.",
	...
	if len(missingGuards) > 0 {
		findings = append(findings, finding{
			...
			action:    "Do not run another release build with the listed Warpctl copies. Install a clean Warpctl containing Warp 217392e or later on the release builder and every enabled managed-services host, preserving §8.11 worker-freshness handling, then rebuild rather than retag affected artifacts.",
