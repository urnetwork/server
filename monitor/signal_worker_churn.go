...
func (workerChurnProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	for _, worker := range pairedWorkers {
		...
		if scoreActive {
			...
			action = "Deploy the target-oriented UpdateClientScores fanout: encode each unchanged target payload once and write the same immutable bytes under the caller-specific Redis keys, while independently encoding only callers whose blocked networks actually remove a provider. Retain the bounded streaming batches; do not raise the cgroup limit or restart to erase the evidence."
			verify = "A post-deploy UpdateClientScores run completes inside its historical band while its exact executor remains below the CPU/allocation guards for two consecutive probes; score keys and provider-selection population remain complete for both unfiltered and genuinely filtered callers."
		}
