...
func emailAssetFindings(domain string, results []emailAssetResult) []finding {
	...
	if edgeMissing > 0 {
		...
		action = "Build and deploy only the Web service from a clean Mmm descendant of b4b229c5c and a clean, attributable SDK checkout. Require the Makefile's sync-public target to place both images in staged output before the fail-closed Warp build. Keep API and Taskworker artifacts containing server commit 7c852d56 or later behind this Web gate; do not copy files into live containers."
