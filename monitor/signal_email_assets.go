...
func emailAssetFindings(domain string, results []emailAssetResult) []finding {
	...
	context := "These absolute URLs are the header wordmark of every transactional email (authentication, password, welcome, payout, data code, and receipt). A 404 produces a broken header in the recipient's mail client and is a functional product regression, not generic scanner traffic. The required asset list is maintained beside this probe and must track controller/email_templates."
	...
	if edgeMissing > 0 {
		...
		action = "Build and deploy only the Web service from a clean Mmm descendant of b4b229c5c. Require the Makefile's sync-public target to place both images in staged output before the fail-closed Warp build; do not copy files into live containers."
