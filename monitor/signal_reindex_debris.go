...
func (reindexDebrisProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	return []finding{{
		...
		action:   "First let any protected measurement or in-progress rebuild reach its bounded outcome. With no active index operation, use §8.12 and §8.13 to compare every active Taskworker artifact with server commits 908a8b2c and d8392c83. Deploy only blocks that predate either fix; those commits exclude the large/high-churn contract and escrow tables from full-table reindex, perform cleanup before and after each permitted object, and keep DbMaintenance owned when only its pooled timestamp refresh stalls. They are the current-main, patch-identical equivalents of former commits 7676014f and abfd976b. When all blocks already contain both fixes, do not redeploy Taskworker: with explicit database-maintenance authorization, proceed to the supported cleanup-only full cycle (`bringyourctl db maintenance all --cleanup`). Do not wildcard-drop indexes or cancel a live rebuild to clear this alert.",
		verify:   "§8.12 proves every active Taskworker's source/digest identity and the runtime behavior of both fixes; the catalog contains zero inactive _ccnew/_ccold artifacts; no excluded large/high-churn table appears in later full-table maintenance progress; and one complete post-deploy maintenance cycle creates no new debris or clustered WAL/storage wait incident.",
		playbook: "SIGNALS.md §2.2a and §2.2",
	}}, nil
}
