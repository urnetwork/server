...
// report against the real key store and the real signature check. The latency
// report is the ingest the ping report supersedes (connect/GEOMAP.md D14); it
// is kept, and tested, for the one release extenders on the previous binary
// still post to it.

// The hint reads no database: the address the request arrived from, placed by
// GeoLite2 and mapped to its continent the way the geo dns maps it. It is the
// same answer the record's continent tag gives (SignExtenderRecord).
func TestExtenderHintPlacesTheCallerAddress(t *testing.T) {
	ctx := context.Background()

	// an address of MaxMind's published test data, which the deployed
	// GeoLite2 build places in the United States
	clientSession := session.NewLocalClientSession(ctx, net.JoinHostPort("67.43.156.1", "443"), nil)
...
func newTestLatencyAttestation(
	...
	t.Helper()
	return newTestLatencyAttestationAt(t, attestor, extender, rttMs, uint64(server.NowUtc().UnixMilli()))
}

// The same, signed with the given timestamp.
func newTestLatencyAttestationAt(
	t testing.TB,
	attestor *connect.ExtenderProbeAttestor,
	extender *model.NetworkExtender,
	rttMs uint32,
...
// The json the previous extender binary posts: a provider claim is a provider
// ping, so the transport of the ping report encodes it field for field.
func testLatencyAttestationArgs(attestation *protocol.ExtenderProbeAttestation) *ExtenderLatencyAttestationArgs {
	transport := connect.ExtenderPingReportFromProto(attestation, connect.ExtenderPingUnknown, nil)
	return &ExtenderLatencyAttestationArgs{
		ClientId:             transport.PingerClientId,
		ExtenderPublicKeyHex: transport.TargetExtenderPublicKeyHex,
...
// A verified attestation is stored as the provider signed it, a replay is not
// a second sample, and the rows stay apart from the pinger-reported ones
...
func TestExtenderLatencyReportStoresWhatVerifies(t *testing.T) {
	server.DefaultTestEnv().Run(t, func(t testing.TB) {
		...
		// the target-reported rows are kept apart from the pinger-reported
		// ones: the dashboard counts the network_ping tallies, and the solver
		// reads these as unknown-verdict rows until the retention drains
		// them (D14)
		connect.AssertEqual(t, model.CountExtenderPings(ctx, server.NowUtc()).Pings24h, int64(0))
		connect.AssertEqual(t, len(model.GetNetworkPings(ctx, extender.ExtenderId, time.Time{})), 0)
...
func TestExtenderLatencyReportRejectsWhatDoesNotVerify(t *testing.T) {
	server.DefaultTestEnv().Run(t, func(t testing.TB) {
		...
		cases := map[string]func() *ExtenderLatencyAttestationArgs{
			...
			// signed by the provider, so it verifies, but no round trip is
			// this long and the column could not hold it: rejected rather
			// than failing the report
			"unstorable rtt": func() *ExtenderLatencyAttestationArgs {
				_, args := newTestLatencyAttestation(t, attestor, extender, math.MaxUint32)
				return args
			},
			"a timestamp two days old": func() *ExtenderLatencyAttestationArgs {
				_, args := newTestLatencyAttestationAt(t, attestor, extender, 100, uint64(server.NowUtc().Add(-48*time.Hour).UnixMilli()))
				return args
			},
			"a timestamp far in the future": func() *ExtenderLatencyAttestationArgs {
				_, args := newTestLatencyAttestationAt(t, attestor, extender, 100, math.MaxUint64)
				return args
			},
		}
...
// An empty report does nothing, an oversized one is refused whole, and the
// rate limit holds the caller after its budget.
func TestExtenderLatencyReportBounds(t *testing.T) {
	server.DefaultTestEnv().Run(t, func(t testing.TB) {
		...
		attestations := make([]*ExtenderLatencyAttestationArgs, settings.MaxCount+1)
		...
		for i := 0; i < settings.RateLimit; i += 1 {
			result, err = ExtenderLatencyReport(&ExtenderLatencyReportArgs{
