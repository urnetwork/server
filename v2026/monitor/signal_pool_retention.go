...
func (poolRetentionProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	return []finding{{
		...
		baseline:  "Idle loopback backends consume less than 50% of the ordinary-role ceiling, excess pools drain toward their configured warm minimum after 600 idle seconds, and §1.3a retains both more than 25% normal-role headroom and more than 64 normal slots.",
		observed:  observed,
		...
		action:   "First read the selected pool settings from every live shard and take one privileged socket census; do not infer PgBouncer ownership from loopback alone. If server_idle_timeout is zero, after protected database maintenance is empty and with explicit operational authorization, deploy from an intentional local Xops checkout containing the 31ae1e7 idle-drain change (deliberate local changes are allowed) with xops/main/ansible/run-dbs.sh --pgbouncer-only; it reloads only changed PgBouncer units and requires their PIDs to remain unchanged. If 600 is already effective and no idle backend is yet 600 seconds old, make no change and observe one complete timeout interval. If the reserve remains consumed after that interval, use SHOW POOLS/STATS and wait metrics to bound default_pool_size, or add database hardware as part of an explicit connection-and-memory budget. Do not restart PostgreSQL/PgBouncer, terminate sessions, or raise max_connections to silence this alert.",
		verify:   "All 32 live shard files report server_idle_timeout=600; the isolated deployment proves every PgBouncer PID is unchanged across reload; after more than 600 seconds outside a demand peak, excess pools contract toward the 256-connection warm floor, both more than 25% normal-role headroom and more than 64 normal slots remain for ten minutes, and neither query_wait_timeout nor rejected server login recurs.",
