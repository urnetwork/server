...
func panicOwnerTestLine(t *testing.T, errorText string, functions ...string) string {
	...
	for _, function := range append([]string{
		"github.com/urnetwork/server/v2026.HandleError.func1",
		"github.com/urnetwork/server/v2026.dbWithPool.func1.1",
		"github.com/urnetwork/server/v2026.Raise",
		"github.com/urnetwork/server/v2026.RaisePgResult[...]",
	}, functions...) {
...
func TestPanicOwnerLongErrorRetainsPrivateSafeDiscriminator(t *testing.T) {
	...
	line := panicOwnerTestLine(t,
		...
		"github.com/urnetwork/server/v2026/model.StampTopLevelClientContractTime.func1.1",
	)
	for range 5 {
...
func TestPanicOwnerMixedThreePlusTwoPreservesAggregate(t *testing.T) {
	...
	for index, owner := range []string{"SyntheticFirst", "SyntheticSecond"} {
		line := panicOwnerTestLine(t, "*errors.errorString=synthetic-secret", "github.com/urnetwork/server/v2026/model."+owner)
		for range 3 - index {
			tailer.classify(line)
		}
	}
...
func TestPanicOwnerNormalizationAndSampleSeparation(t *testing.T) {
	...
	for _, wrapper := range []string{
		...
		line := panicOwnerTestLine(t, "*errors.errorString=synthetic-secret", "github.com/urnetwork/"+wrapper,
			"github.com/urnetwork/server/v2026/model.SyntheticWrite")
	...
	for _, owner := range []string{"SyntheticFirst", "SyntheticSecond"} {
		for index := range 5 {
			tailer.classify(panicOwnerTestLine(t, "*errors.errorString=synthetic-secret",
				fmt.Sprintf("github.com/urnetwork/server/v2026/model.%s.func%d.1", owner, index+1)))
		}
	}
	alerts := panicOwnerTestAlerts(t, tailer)
...
func TestPanicOwnerMalformedAndOversizedStayAggregateOnly(t *testing.T) {
	valid := panicOwnerTestLine(t, "*errors.errorString=synthetic-secret", "github.com/urnetwork/server/v2026/model.SyntheticWrite")
	for _, line := range []string{
		...
		`Unexpected error: {"error":"synthetic-secret","stack":["github.com/urnetwork/server/v2026/model.SyntheticWrite(0x12345678)"]}`,
		...
		panicOwnerTestLine(t, "*errors.errorString=synthetic-secret", "github.com/urnetwork/server/v2026/model.SyntheticWrite[unrecognized]",
			"github.com/urnetwork/server/v2026/model.SyntheticCaller"),
		panicOwnerTestLine(t, "*errors.errorString=synthetic-secret", "github.com/urnetwork/server/v2026/model."+strings.Repeat("Synthetic", 30)),
	...
	stack[0] = "github.com/urnetwork/server/v2026/model.SyntheticWrite(0x12345678)"
...
func TestPanicOwnerUnknownFirstSampleAndTypeGuards(t *testing.T) {
	...
	tailer.classify(panicOwnerTestLine(t, "*errors.errorString=synthetic-secret", "github.com/urnetwork/server/v2026/model.SyntheticWrite"))
	alerts := panicOwnerTestAlerts(t, tailer)
	...
	for _, errorText := range []string{
		...
		observation := parsePanicLogObservation(panicOwnerTestLine(t, errorText, "github.com/urnetwork/server/v2026/model.SyntheticWrite"))
...
func TestPanicOwnerCardinalityCapResetsWithoutSuppressingAggregate(t *testing.T) {
	...
	lineForOwner := func(index int) string {
		return panicOwnerTestLine(t, "*errors.errorString=synthetic-secret",
			fmt.Sprintf("github.com/urnetwork/server/v2026/model.SyntheticOwner%d", index))
	}
