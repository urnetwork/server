...
func TestLogErrorsSignalExplainsLokiTailDroppedStreams(t *testing.T) {
	...
	for _, detail := range []string{
		...
		"Warp commit 13fcd05",
		"stops Mimir routine info at the source",
		"retaining warning/error evidence",
		"every minute instead of leaving near-15-minute phase skew",
...
func TestLogErrorsSignalExplainsLokiTailBackendEOF(t *testing.T) {
	...
	for _, detail := range []string{
		...
		"Warp commit 13fcd05",
		"stops Mimir 3.1.1 routine info at the source",
		"refreshes the single-tenant store-gateway bucket index every minute",
		"moves Loki's per-query table lookup record to debug",
		"Do not claim producer reduction itself fixes loss",
...
func TestLogErrorsSignalExplainsPayoutWalletInsufficiency(t *testing.T) {
	...
	for _, detail := range []string{
		...
		"artifact containing current-main server commit 66525afc",
