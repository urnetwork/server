...
func (self *egressAdmissionProbe) observe(now time.Time, processes map[egressAdmissionIdentity]*egressAdmissionProcess) []finding {
	...
	if unfunded := deltas["pass_error/funding_unavailable"]; unfunded > 0 {
		findings = append(findings, finding{
			...
			action:    "Compare bounded durable outstanding reservations with the Redis mirror and inspect expired-contract cleanup and the internal prober's conditional shard preflight/bootstrap. Split new companion versus own-source reservation size. Distinguish genuinely reserved credit from stale mirror state. Do not manually credit user accounts, release escrow, delete verdicts or weaken provider gates from this alert.",
