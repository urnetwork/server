...
var logClasses = []logClass{
	...
	{name: "conn-reset", re: regexp.MustCompile(`connection reset by peer|unexpected EOF`),
		...
		context:   "On 2026-09-03, Fireside Proxy emitted 87-158/min device_rpc_transport WebSocket close-1006 unexpected-EOF lines while the focused proxy-memory, proxy-pool, proxy-runtime, and proxy-cache controls were all healthy. A redacted ten-minute cohort contained 1,176 attach/EOF/detach triplets from one logical hosted device: 1,075 of 1,175 measured sessions lasted under one second, 100 lasted one to four seconds, and none reached five seconds. There were zero write, keepalive, receive-budget, receive-queue, version-rejection, instance-rejection, or SyncReverse markers. Proxy accepts this /device-rpc WebSocket and the remote DeviceRemote initiates it; the exact 500-millisecond pace matches failed-sync retry pacing and localizes this wave to one early remote-to-local sync lifecycle without yet identifying why sync failed. The contemporaneous controls reject Proxy resource pressure as the demonstrated cause; they do not make those controls universally irrelevant.",
		action:    "Identify whether the emitter accepted or initiated the connection, then inspect the exact peer and path for process-generation changes, orderly-close evidence, load-balancer/NAT/conntrack loss, carrier errors, OOM/cgroup pressure, socket pressure, and provider or device lifecycle. For the current Fireside cohort, preserve the redacted one-device cardinality, compare the exact remote and Proxy RPC wire generations, and capture a bounded early-sync stage/result on each endpoint; the existing server logs do not distinguish request decode, response decode, client cancellation, or another pre-SyncReverse failure. Do not call the event a server close, restart a healthy process, increase buffers, or attribute it to memory without a matching direct signal.",
