...
func (provenanceProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	if len(missing) > 0 {
		findings = append(findings, finding{
			...
			action:   "Deploy the generic source-info collector in the affected service from a clean release built by the fail-closed Warp builder, then let ordinary host-serialized rollout replace the legacy generation. Do not substitute a tag, config generation, git checkout, or BuildKit context attestation for the running executable and container identities.",
