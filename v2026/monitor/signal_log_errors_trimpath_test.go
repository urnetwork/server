...
func panicOwnerTrimpathTestLine(t *testing.T, innerFunction, innerSource string) string {
	...
	stack := []string{
		...
		"github.com/urnetwork/server/v2026.HandleError.func1(0x12345678)",
		"github.com/urnetwork/server/v2026/trace.go:58 +0x85",
		"panic({0x12345678?, 0x87654321?})",
		"runtime/panic.go:860 +0x13a",
		"github.com/urnetwork/server/v2026.dbWithPool.func1.2(0x12345678)",
		"github.com/urnetwork/server/v2026/db.go:619 +0x1a5",
		"github.com/urnetwork/server/v2026.txWithPool.func1.1(0x12345678)",
		"github.com/urnetwork/server/v2026/db.go:767 +0x74",
		"github.com/urnetwork/server/v2026.Raise(...)",
		"github.com/urnetwork/server/v2026/util.go:154",
		innerFunction + "(0x12345678, 0x87654321)",
		innerSource,
		"github.com/urnetwork/server/v2026/model.SyntheticOuter(0x12345678)",
		"github.com/urnetwork/server/v2026/model/synthetic_outer.go:12345 +0x99",
	}
...
func TestPanicOwnerTrimpathWrappersRetainApplicationOwner(t *testing.T) {
	line := panicOwnerTrimpathTestLine(t,
		"github.com/urnetwork/server/v2026/model.ForceCloseOpenContractIds.func5.1",
		"github.com/urnetwork/server/v2026/model/subscription_model.go:3980 +0x296",
	)
...
func TestPanicOwnerVersionedRootSkipsRecoveryWrapper(t *testing.T) {
	line := panicOwnerTrimpathTestLine(t,
		"github.com/urnetwork/server/v2026/model.SyntheticWrite.func1",
		"github.com/urnetwork/server/v2026/model/synthetic_write.go:123 +0x99",
	)
	line = strings.ReplaceAll(line, "github.com/urnetwork/server/v2026", "github.com/urnetwork/server/v2026")
	observation := parsePanicLogObservation(line)
...
func TestPanicOwnerTrimpathUnknownInnerDoesNotBorrowCaller(t *testing.T) {
	for _, inner := range []string{
		"github.com/urnetwork/server/v2026/model.SyntheticUnknown[not-supported]",
		"github.com/urnetwork/unlisted.SyntheticUnknown",
	} {
		line := panicOwnerTrimpathTestLine(t, inner,
			"github.com/urnetwork/server/v2026/model/synthetic_owner.go:12345 +0x99")
...
func TestPanicOwnerTrimpathMalformedSourceRemainsUnknown(t *testing.T) {
	for _, source := range []string{"", "synthetic-secret", "github.com/urnetwork/server/v2026/model/owner.go:unknown"} {
		line := panicOwnerTrimpathTestLine(t,
			"github.com/urnetwork/server/v2026/model.ForceCloseOpenContractIds.func5.1", source)
		if got := panicLogOwner(line); got != "" {
	...
	valid := panicOwnerTrimpathTestLine(t,
		"github.com/urnetwork/server/v2026/model.ForceCloseOpenContractIds.func5.1",
		"github.com/urnetwork/server/v2026/model/subscription_model.go:3980 +0x296")
	brokenWrapper := strings.Replace(valid, "github.com/urnetwork/server/v2026/trace.go:58 +0x85", "synthetic-secret", 1)
