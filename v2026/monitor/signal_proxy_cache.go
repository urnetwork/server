...
func (proxyCacheProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	if len(missing) > 0 {
		findings = append(findings, finding{
			...
			action:   "Provenance-check and deploy the Proxy artifact from an intentional local server checkout containing current-main commit a11ae7b1, which contains the bounded TTL/LRU cache and identity-free cache metrics, through the ordinary host-serialized rollout. Record any participating diff. Do not restart solely to hide retained entries or infer zero entries from an absent gauge.",
			verify:   "Every newest identity reports a valid source/digest identity attributable to a checkout containing a11ae7b1 and exports all eight joined metrics for two consecutive scrapes; capacity is positive and no greater than 16,384, and entries never exceed capacity.",
