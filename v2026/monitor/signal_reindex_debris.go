...
func (reindexDebrisProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	return []finding{{
		...
		action:   "First let any protected measurement or in-progress rebuild reach its bounded outcome. With no active index operation, satisfy §8.13 and deploy Taskworker from a clean server descendant containing 7676014f and abfd976b: it excludes the large/high-churn contract and escrow tables from full-table reindex, performs cleanup before and after each permitted object, and keeps DbMaintenance owned when only its pooled timestamp refresh stalls. Then, with explicit database-maintenance authorization, run the supported cleanup-only full cycle (`bringyourctl db maintenance all --cleanup`). Do not wildcard-drop indexes or cancel a live rebuild to clear this alert.",
