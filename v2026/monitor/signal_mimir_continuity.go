...
func (mimirContinuityProbe) check(ctx context.Context, env *probeEnv) ([]finding, error) {
	...
	return []finding{{
		...
		action:   "Run the §11.21 mimir-shutdown signal. If any exact child reports false, deploy a Grafana image from an intentional local Warp checkout containing commit 7176ccd while retaining the parent's 120-second Mimir child stop allowance inside Warpctl's 3,600-second container drain. If every current child already reports true, do not redeploy solely for historical gaps; preserve the setting through the next ordinary Grafana rollout and use that shutdown as the discriminator. Do not zero-fill or span over the panel, and do not shared-mount one TSDB directory into overlapping old and new containers.",
