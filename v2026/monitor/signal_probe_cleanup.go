...
func probeCleanupLeakFinding(snapshot probeCleanupSnapshot, severity string) finding {
	return finding{
		...
		action:    "Build and deploy Taskworker from immutable sibling inputs containing Connect d3b49d9 and Operator Proxy 35b0bc7, and prove those exact trees in the image manifest; the outer Server VCS stamp is insufficient when local module replacements are used. Do not delete or deactivate production rows merely to clear this signal, and do not call a small or empty cohort recovery while §2.19 probe activity is insufficient.",
