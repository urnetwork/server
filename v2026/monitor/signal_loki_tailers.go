...
func lokiTailAccountingInvalidFinding(host string, instances int, invalid []string) finding {
	return finding{
		...
		mechanism: "Loki 3.7.3 increments each tail gauge once when it constructs a Tailer. At the one-hour tail_max_duration or after every ingester connection is lost, the tail loop calls close; the HTTP handler then unconditionally runs its deferred close. Tailer.close is not idempotent and decrements both gauges on every call, so one real tail can be subtracted twice. The same close path remains non-idempotent in the official Loki main source inspected on 2026-08-31. The sole v163 watcher started eight service tails at 23:50:39Z; edge-0 first underflowed one lifecycle later at 00:51:27Z to tails=-8 and streams=-9. Starting the sole v164 watcher added eight live tails to that unchanged Loki process and moved the gauges to 0/-1, proving active tails can numerically mask earlier over-decrements without repairing them.",
