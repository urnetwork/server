...
// Flush reports the current buckets immediately. Called once at drain end so
// the final window's requests leave a record instead of exiting with the
// process (APIDRAIN1.md §2.5).
func (self *RouterStats) Flush() {
	self.report()
}

func (self *RouterStats) report() {
