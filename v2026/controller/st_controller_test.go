...
func TestStComputePayoutSharesAggregatesPerColdkey(t *testing.T) {
	...
	connect.AssertEqual(t, otherColdkey, shares[0].Coldkey)
	connect.AssertEqual(t, sharedColdkey, shares[1].Coldkey)
	...
	connect.AssertEqual(t, lowId, shares[1].NetworkId)
}
...
func TestStComputePayoutSharesSkips(t *testing.T) {
	...
	connect.AssertEqual(t, 0, len(stComputePayoutShares(nil, 8)))
	...
	connect.AssertEqual(t, 0, len(shares))
...
func TestStComputePayoutSharesDeterministic(t *testing.T) {
	...
	connect.AssertEqual(t, len(forwardShares), len(reversedShares))
	bpsTotal := 0
	for i := range forwardShares {
		connect.AssertEqual(t, forwardShares[i].Coldkey, reversedShares[i].Coldkey)
		connect.AssertEqual(t, forwardShares[i].ShareBps, reversedShares[i].ShareBps)
...
func TestStBuildShareEntriesExcludesHeadBound(t *testing.T) {
	...
	connect.AssertEqual(t, 2, len(entries))
	for _, entry := range entries {
		connect.AssertNotEqual(t, netB, entry.NetworkId)
		connect.AssertNotEqual(t, coldkeyB, entry.Coldkey)
	}
	...
	bpsTotal := 0
	for _, share := range shares {
		connect.AssertNotEqual(t, coldkeyB, share.Coldkey)
		bpsTotal += share.ShareBps
	}
	connect.AssertEqual(t, 5000, shares[0].ShareBps)
	connect.AssertEqual(t, 5000, shares[1].ShareBps)
	...
	connect.AssertEqual(t, 3, len(allEntries))
...
func TestStEstimateBlockTime(t *testing.T) {
	...
	connect.AssertEqual(t, headTime.Add(1200*time.Second), stEstimateBlockTime(headBlock, headTime, 1100, 12))
	connect.AssertEqual(t, headTime.Add(-1200*time.Second), stEstimateBlockTime(headBlock, headTime, 900, 12))
	connect.AssertEqual(t, headTime, stEstimateBlockTime(headBlock, headTime, 1000, 12))

	// a non-positive cadence falls back to the ~12s default
	connect.AssertEqual(t, headTime.Add(120*time.Second), stEstimateBlockTime(headBlock, headTime, 1010, 0))
}
...
func TestStBuildPayoutTreeSingleLeaf(t *testing.T) {
	...
	hashedLeaf := merkle.PayoutLeaf(leaf.Coldkey, big.NewInt(int64(leaf.ShareBps)))
	connect.AssertEqual(t, [32]byte(hashedLeaf), tree.Root())
	proof, err := tree.ProofAt(0)
	connect.AssertEqual(t, nil, err)
	connect.AssertEqual(t, 0, len(proof))
	connect.AssertEqual(t, true, merkle.Verify(tree.Root(), hashedLeaf, proof))
