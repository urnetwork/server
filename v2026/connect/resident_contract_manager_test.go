...
func TestCreateContract(t *testing.T) { server.DefaultTestEnv().Run(t, func(t testing.TB) {
    ...
    for _, networkId := range []server.Id{networkIdA, networkIdB} {
	    ...
	    connect.AssertEqual(t, true, success)
	}
	...
	activeTransferBalance = model.GetActiveTransferBalanceByteCount(ctx, networkIdB)
	connect.AssertEqual(t, initialTransferBalance - 2 * contractTransferByteCount, activeTransferBalance)
