...
func ExampleULID() {
	...
	fmt.Println(MustNew(Timestamp(t), entropy))
...
func TestNew(t *testing.T) {
	...
	t.Run("ULID", testULID(func(ms uint64, e io.Reader) ULID {
		id, err := New(ms, e)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Error", func(t *testing.T) {
		_, err := New(MaxTime()+1, nil)
		if got, want := err, ErrBigTime; got != want {
			t.Errorf("got err %v, want %v", got, want)
		}

		_, err = New(0, strings.NewReader(""))
...
func TestMustNewDefault(t *testing.T) {
	...
	t.Run("Panic", func(t *testing.T) {
		defer func() {
			if got, want := recover(), ErrBigTime; got != want {
				t.Errorf("got panic %v, want %v", got, want)
			}
		}()
		_ = MustNewDefault(time.Time{})
	})
}
...
func TestMustParse(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"MustParse", MustParse},
		{"MustParseStrict", MustParseStrict},
	} {
		t.Run(tc.name, func(t *testing.T) {
			defer func() {
				if got, want := recover(), ErrDataSize; got != want {
					t.Errorf("got panic %v, want %v", got, want)
				}
			}()
			_ = tc.fn("")
...
func testULID(mk func(uint64, io.Reader) ULID) func(*testing.T) {
	return func(t *testing.T) {
		want := ULID{0x0, 0x0, 0x0, 0x1, 0x86, 0xa0}
...
func TestRoundTrips(t *testing.T) {
	...
	prop := func(id ULID) bool {
		...
		return id == a && b == id &&
			id == MustParse(id.String()) &&
			id == MustParseStrict(id.String())
	}
...
func TestMarshalingErrors(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"UnmarshalBinary", id.UnmarshalBinary, ErrDataSize},
		{"UnmarshalText", id.UnmarshalText, ErrDataSize},
		{"MarshalBinaryTo", id.MarshalBinaryTo, ErrBufferSize},
		{"MarshalTextTo", id.MarshalTextTo, ErrBufferSize},
	} {
		t.Run(tc.name, func(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != ErrInvalidCharacters {
				t.Errorf("Parse(%q): got err %v, want %v", tt.input, err, ErrInvalidCharacters)
			}
		})
	}
}
...
func TestAlizainCompatibility(t *testing.T) {
	...
	got := MustNew(ts, bytes.NewReader(make([]byte, 16)))
	want := MustParse("01ARYZ6S410000000000000000")
	if got != want {
...
func TestEncoding(t *testing.T) {
	...
	enc := make(map[rune]bool, len(Encoding))
...
func TestLexicographicalOrder(t *testing.T) {
	...
	top := MustNew(MaxTime(), nil)
	for i := 0; i < 10; i++ { // test upper boundary state space
		next := MustNew(top.Time()-1, nil)
		if !prop(top, next) {
...
func TestCaseInsensitivity(t *testing.T) {
	...
	upper := func(id ULID) (out ULID) {
		return MustParse(strings.ToUpper(id.String()))
	}

	lower := func(id ULID) (out ULID) {
		return MustParse(strings.ToLower(id.String()))
	}
...
func TestParseRobustness(t *testing.T) {
	...
	for _, tc := range cases {
		if _, err := Parse(string(tc)); err != nil {
			t.Error(err)
		}
	}
...
func TestNow(t *testing.T) {
	...
	before := Now()
	after := Timestamp(time.Now().UTC().Add(time.Millisecond))
...
func TestTimestamp(t *testing.T) {
	...
	if got, want := Timestamp(tm), uint64(1000); got != want {
...
func TestTime(t *testing.T) {
	...
	original := time.Now()
	diff := original.Sub(Time(Timestamp(original)))
...
func TestULIDTime(t *testing.T) {
	...
	var id ULID
	if got, want := id.SetTime(maxTime+1), ErrBigTime; got != want {
		t.Errorf("got err %v, want %v", got, want)
	}
...
func TestEntropy(t *testing.T) {
	...
	var id ULID
	if got, want := id.SetEntropy([]byte{}), ErrDataSize; got != want {
		t.Errorf("got err %v, want %v", got, want)
	}
...
func TestOverflowHandling(t *testing.T) {
	...
	for s, want := range map[string]error{
		...
		"80000000000000000000000000": ErrOverflow,
		"80000000000000000000000001": ErrOverflow,
		"ZZZZZZZZZZZZZZZZZZZZZZZZZZ": ErrOverflow,
	} {
		if _, have := Parse(s); want != have {
...
func TestMonotonic(t *testing.T) {
	...
	for _, e := range []struct {
		...
		for _, inc := range []uint64{
			...
			inc := inc
			entropy := Monotonic(e.mk(), uint64(inc))
...
func TestMonotonicOverflow(t *testing.T) {
	...
	prev, err := New(0, entropy)
	if err != nil {
		t.Fatal(err)
	}

	next, err := New(prev.Time(), entropy)
	if have, want := err, ErrMonotonicOverflow; have != want {
...
func TestMonotonicSafe(t *testing.T) {
	...
	var (
		rng  = rand.New(rand.NewSource(time.Now().UnixNano()))
		safe = &LockedMonotonicReader{MonotonicReader: Monotonic(rng, 0)}
...
func TestULID_Bytes(t *testing.T) {
	...
	entropy := Monotonic(rand.New(rand.NewSource(tt.UnixNano())), 0)
	id := MustNew(Timestamp(tt), entropy)
	bid := id.Bytes()
	bid[len(bid)-1]++
...
func benchmarkMakeULID(b *testing.B, f func(uint64, io.Reader)) {
	...
	for _, tc := range []struct {
		...
		{"WithMonotonicEntropy_SameTimestamp_Inc0", []uint64{123}, Monotonic(rng, 0)},
		{"WithMonotonicEntropy_DifferentTimestamp_Inc0", []uint64{122, 123}, Monotonic(rng, 0)},
		{"WithMonotonicEntropy_SameTimestamp_Inc1", []uint64{123}, Monotonic(rng, 1)},
		{"WithMonotonicEntropy_DifferentTimestamp_Inc1", []uint64{122, 123}, Monotonic(rng, 1)},
		{"WithCryptoMonotonicEntropy_SameTimestamp_Inc1", []uint64{123}, Monotonic(crand.Reader, 1)},
		{"WithCryptoMonotonicEntropy_DifferentTimestamp_Inc1", []uint64{122, 123}, Monotonic(crand.Reader, 1)},
...
func BenchmarkUnmarshal(b *testing.B) {
	...
	txt := []byte(s)
	bin, _ := MustParse(s).MarshalBinary()
...
func BenchmarkEntropy(b *testing.B) {
	id := MustNew(0, strings.NewReader("ABCDEFGHIJKLMNOP"))
	b.SetBytes(10)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = id.Entropy()
...
func BenchmarkCompare(b *testing.B) {
	id, other := MustNew(12345, nil), MustNew(54321, nil)
